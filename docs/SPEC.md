...
**Prompt delivery (§7.4 addendum).** Every prompt is copied to the clipboard before any open path runs. The Claude Code extension cannot seed a composer that is already open — it reveals the panel and shows "Session is already open. Your prompt was not applied — enter it manually." — and exposes no command to set an open composer's text (verified against 2.1.270: `claude-vscode.focus` only delivers the editor selection as an @-mention; VS Code 1.137 has no webview paste command). So `SessionOpener` first looks for an open Claude panel tab (`TabInputWebview.viewType === 'mainThreadWebview-claudeVSCodePanel'`) whose label equals the session's title (Claude labels the tab with the same custom/AI title the index resolves); when found it calls `editor.open` **without** the prompt — which just reveals the panel and shows no message — then runs `claude-vscode.focus` to put the caret in the composer and shows one hint to paste. When no tab matches by title it passes the prompt and counts panel tabs for up to 1.5 s; no new tab ⇒ same focus-and-paste hint (Claude's own "already open" message also appears in that case).
